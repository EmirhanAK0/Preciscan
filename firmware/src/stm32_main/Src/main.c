...
int main(void)
{
  ...
  uart_comm_println("\r\n=================================");
  uart_comm_println("PRECISCAN SYSTEM BOOT OK!");
  uart_comm_println("WAITING FOR COMMANDS...");
  uart_comm_println("=================================");
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure Laser Trigger Pin (PC2) Output Level */
  ...
  /*Configure Laser Trigger Pin (PC2) */
...
/* TX tamamlandi — ring buffer'dan bir sonraki byte'i gonder */
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == USART2)
    {
        uart_comm_tx_callback();
    }
}
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart)
{
    ...
    {
        extern uint8_t rx_byte;
        /* Hata durumunda (Overrun vb.) RX'i yeniden baslat */
        HAL_UART_Receive_IT(huart, &rx_byte, 1);
    }
}
/* USER CODE END 4 */
