...
{
    if ((a.x < targetX && b.x < targetX) || (a.x > targetX && b.x > targetX))
        return false;
    if (std::abs(a.x - b.x) < 1e-9f)
        return false;
...
std::vector<ProfilePoint> sliceAtAngle(const Mesh& mesh, float theta_rad, const SliceParams& params)
{
    ...
    {
        Vec3f rv[3] = {rotateZ(tri.v[0], cosA, sinA), rotateZ(tri.v[1], cosA, sinA),
        ...
        if (tmaxZ < 0.0f || tminZ > 22.0f)
            continue;

        Vec3f hits[3];
        int n = 0;
        Vec3f h;
        if (intersectEdgeX(rv[0], rv[1], 0.0f, h))
            hits[n++] = h;
        if (intersectEdgeX(rv[1], rv[2], 0.0f, h))
            hits[n++] = h;
        if (intersectEdgeX(rv[2], rv[0], 0.0f, h))
            hits[n++] = h;
        ...
        if (n >= 2)
        {
            ...
            for (float z = std::ceil(startZ * Z_SCALE) / Z_SCALE; z <= endZ; z += 0.2f)
            {
                ...
                float y  = y0 + (y1 - y0) * t;
                // Look from +Y
                if (zMap.find(zIdx) == zMap.end() || y > zMap[zIdx])
                    zMap[zIdx] = y;
            }
        }
    }
