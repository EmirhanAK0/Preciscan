...
void ProfileWidget::paintEvent(QPaintEvent*)
{
    ...
    for (int i = 0; i <= 5; ++i)
    {
        ...
        p.drawText(QRectF(sp.x() - 12, m_plotArea.bottom() + 4, 24, 14), Qt::AlignCenter,
                   QString::number((int)h));
    }
    ...
    p.setFont(titleFont);
    p.drawText(QRectF(m_plotArea.left(), height() - 14, m_plotArea.width(), 14), Qt::AlignCenter,
               "Height (mm)");
    // Y label (rotasyonlu)
    p.save();
