...
SetupPanel::SetupPanel(ScanController* ctrl, QWidget* parent) : QWidget(parent), m_ctrl(ctrl)
{
    ...
    group->setStyleSheet("QGroupBox { color: #aaa; font-weight: bold; border: 1px solid #333; "
    ...
    m_dOffsetSpin->setStyleSheet("background: #1a1a1a; color: #ccc; border: 1px solid #333; "
                                 "border-radius: 4px; padding: 4px;");
    connect(m_dOffsetSpin, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
            &SetupPanel::onDOffsetChanged);
    ...
    m_resSpin->setStyleSheet("background: #1a1a1a; color: #ccc; border: 1px solid #333; "
                             "border-radius: 4px; padding: 4px;");
    connect(m_resSpin, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
            &SetupPanel::onResolutionChanged);
...
{
    if (m_ctrl)
        m_ctrl->setDOffset(static_cast<float>(val));
}

void SetupPanel::onResolutionChanged(double val)
{
    if (m_ctrl)
        m_ctrl->setResolution(static_cast<float>(val));
}