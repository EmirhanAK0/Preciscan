...
McuListener::McuListener(int port, IDataSink* sink) : m_port(port), m_sink(sink), m_running(false)
{
}
    ...
    while (m_running)
    {
        ...
        if (select(static_cast<int>(sock) + 1, &readfds, NULL, NULL, &tv) > 0)
        {
            ...
            if (bytes >= (int)sizeof(McuFixedPacket))
            {
                ...
                if (m_sink)
                {
                    m_sink->on_packet(reinterpret_cast<const uint8_t*>(buffer), (size_t)bytes, 0);
                }
                else if (!m_packetCb)
                {
                    // Hic sink yok: minimal konsol log
                    std::cout << "\r[MCU] Tetik: " << pkt.trigger_seq << " Y: " << pkt.y_position
                              << " mm    " << std::flush;
                }
            }
        }
